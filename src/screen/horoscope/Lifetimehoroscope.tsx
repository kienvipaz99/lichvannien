...
import {ApiTuvi, ApiTuviTrondoi} from '../../res/callapi/Api';
...
import {htmltuvitrondoi} from '../../res/converts';

export default function Lifetimehoroscope({
  navigation,
}: {
  ...
  const callData = async () => {
    setLoading(true);
    try {
      const aa = await axios.post(`${ApiTuviTrondoi}`, {
  ...
  return (
    <View style={{flex: 1}}>
      <CustomHeader title="Tử vi trọn đời" back onBackPress={navigation} />
        ...
        <ScrollView
          ...
          <View style={styles.view1}>
            ...
            <View style={styles.view3}>
              <HTML
                source={{
                  html: `<div style="color:black;">${htmltuvitrondoi(
                    data,
                  )}</div>`,
                }}
                contentWidth={sizes.width}
              />
              <Image source={images.tuvitrondoi} style={styles.img} />
              <Text style={styles.txt1}>Tra cứu tử vi trọn đời theo tuổi</Text>
            </View>
          </View>
        </ScrollView>
      </View>
