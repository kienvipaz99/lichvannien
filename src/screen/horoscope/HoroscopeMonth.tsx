...
import {ApiTuvi} from '../../res/callapi/Api';
...
import {htmltuvithang} from '../../res/converts';
import Loading from '../../components/loading/Loading';
import {NavigationProp} from '@react-navigation/native';
import images from '../../res/images';

export default function HoroscopeMonth({
  navigation,
}: {
  ...
  const callData = async () => {
    setLoading(true);
    try {
      const aa = await axios.post(`${ApiTuvi}`, {
  ...
  return (
    <View style={{flex: 1}}>
      <CustomHeader title="Tử vi tháng" back onBackPress={navigation} />
        ...
        <ScrollView
          ...
          <View style={styles.view1}>
            <View style={stylescustom.row2}>
              ...
              <View style={styles.view}>
                <Text style={styles.txt}>Tháng xem (Âm lịch)</Text>
                <TextInPutCT
                  style={styles.txtip}
                  text={monthView}
                  onChangText={setMonthView}
            ...
            <View style={styles.view3}>
              <HTML
                source={{
                  html: `<div style="color:black;">${htmltuvithang(
                    data,
                  )}</div>`,
                }}
                contentWidth={sizes.width}
              />
              <Image source={images.tuvithang} style={styles.img} />
              <Text style={styles.txt1}>
                Tra cứu tử vi tháng, tài vận tháng theo ngày sinh của bạn{' '}
              </Text>
            </View>
          </View>
        </ScrollView>
